...
public class Player_NOKAVBackFar2 extends Player {
	
	private int _id=0;
	private int enemy;
	private int[] hpRemaining = new int[Constants.Max_Units];
	
	public Player_NOKAVBackFar2(int playerID) {
		_id=playerID;
		setID(playerID);
	...
	public void getMoves(GameState  state, HashMap<Integer,List<UnitAction>> moves, List<UnitAction>  moveVec)
	{
		...
		for (Integer u : moves.keySet()){
			closestUnit= null;farthestUnit=null;
			//foundUnitAction=false;
			//actionMoveIndex=0;
			actionHighestDPS=0;
			//closestMoveIndex =0;
			closestMoveDist=Integer.MAX_VALUE;
			int furthestMoveDist					= Integer.MAX_VALUE;
			...
			for (m=0; m<movesForU.size(); m++)
			{
				...
				//if (!foundUnitAction){
					...
					if(farthestUnit == null){
						farthestUnit=state.getFarthestOurUnit(ID(), u);
					}
					...
					else if (move._moveType == UnitActionTypes.MOVE)
					{
						...
						dist					=farthestUnit.getDistanceSqToPosition(ourDest, state.getTime());
