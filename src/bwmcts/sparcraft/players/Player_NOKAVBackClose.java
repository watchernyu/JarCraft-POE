...
public class Player_NOKAVBackClose extends Player {
	
	private int _id=0;
	private int enemy;
	private int[] hpRemaining = new int[Constants.Max_Units];
	
	public Player_NOKAVBackClose(int playerID) {
		_id=playerID;
		setID(playerID);
	...
	public void getMoves(GameState  state, HashMap<Integer,List<UnitAction>> moves, List<UnitAction>  moveVec)
	{
		...
		for (Integer u : moves.keySet()){
			...
			for (m=0; m<movesForU.size(); m++)
			{
				...
				if (actionMove==null){
					Unit closest = state.getClosestOurUnit(ID(), u);
					...
					if (move._moveType == UnitActionTypes.MOVE)
					{
						...
						dist					=closest.getDistanceSqToPosition(ourDest, state.getTime());
