...
public class Player_NOKAVForwardFar2 extends Player {
	
	private int _id=0;
	private int enemy;
	private int[] hpRemaining = new int[Constants.Max_Units];
	
	public Player_NOKAVForwardFar2(int playerID) {
		_id=playerID;
		setID(playerID);
	...
	public void getMoves(GameState  state, HashMap<Integer,List<UnitAction>> moves, List<UnitAction>  moveVec)
	{
		...
		for (Integer u : moves.keySet()){
			...
			for (m=0; m<movesForU.size(); m++)
			{
				...
				//if (!foundUnitAction){
					Unit farthestUnit;
					farthestUnit=state.getFarthestEnemyUnit(_id,u);

					
					if (move._moveType == UnitActionTypes.MOVE)
					{
						
						dist = farthestUnit.getDistanceSqToPosition(ourUnit.pos().getX() + Constants.Move_DirX[move.index()], 
