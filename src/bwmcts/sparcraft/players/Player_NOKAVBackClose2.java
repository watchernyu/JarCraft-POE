...
public class Player_NOKAVBackClose2 extends Player {
	
	private int _id=0;
	private int enemy;
	private int[] hpRemaining = new int[Constants.Max_Units];
	
	public Player_NOKAVBackClose2(int playerID) {
		_id=playerID;
		setID(playerID);
	...
	public void getMoves(GameState  state, HashMap<Integer,List<UnitAction>> moves, List<UnitAction>  moveVec)
	{
		...
		for (Integer u : moves.keySet()){
			...
			UnitAction closestAllyMove=null;
			UnitAction closestMove=null;
			...
			for (m=0; m<movesForU.size(); m++)
			{
				...
				//if (!foundUnitAction){
					...
					if(closestAllyUnit == null){
						closestAllyUnit=state.getClosestOurUnit(ID(), u);
					}
					...
					else if (move._moveType == UnitActionTypes.MOVE)
					{
						...
						dist = closestAllyUnit.getDistanceSqToPosition(ourUnit.pos().getX() + Constants.Move_DirX[move.index()], 
								ourUnit.pos().getY() + Constants.Move_DirY[move.index()], state.getTime());

						if (dist < closestAllyUnitMoveDist)
						{
							closestAllyUnitMoveDist = dist;
							closestAllyMove=move;
						}
					}
				}
			}
