...
//used for experiments related to POE algorithm
//last modified: Nov-6-2016

public class Test_final implements BWAPIEventListener  {
	
	private static boolean DEBUGNOW = true;
	//not useful..
	...
	int totalRuns = 100;//total number of experiments for a particular scenario
	
	public static void main(String[] args) throws Exception{
		System.out.println("Create TC instance");
		Test_final tc=new Test_final();
...
////////IN BETWEEN SET EXPERIMENTS///////////////////////////////////////////////////////////
		...
		if(EXPERIMENT){
			int experimentTimeLimit = 20;
			//set time limit in milliseconds
			
			//startt = System.currentTimeMillis();
			//tc.poe2_vs_pgs_tl(experimentTimeLimit);
			//tc.giveTime(startt);
			
			//startt = System.currentTimeMillis();
			//tc.poe2_vs_guct_tl(experimentTimeLimit);
			//tc.giveTime(startt);
	
			startt = System.currentTimeMillis();
			tc.poe2_vs_suct_tl(experimentTimeLimit);
			tc.giveTime(startt);
			
			startt = System.currentTimeMillis();
			tc.poe6_vs_suct_tl(experimentTimeLimit);
			tc.giveTime(startt);
			
			startt = System.currentTimeMillis();
			tc.pgs_vs_guct_tl(experimentTimeLimit);
			tc.giveTime(startt);
			
			startt = System.currentTimeMillis();
			tc.pgs_vs_suct_tl(experimentTimeLimit);
			tc.giveTime(startt);
...
////////IN BETWEEN SET EXPERIMENTS///////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////////////
	public void poe2_vs_suct_tl(int tl){
		//timelimit is in ms
		Player p1= new Player_POE_final_no_cross(0);
		Player_POE_final_no_cross pe1 = (Player_POE_final_no_cross) p1;
		pe1.init2scripts();
		pe1.setTimeLimit(tl);
		//cast player to 

		Player p2= new UctLogic(bwapi, new IUCTCD(new UctConfig(1)),tl);
	...
	public void pgs_vs_guct_tl(int tl){
		
		Player p1= new Player_PGS(0);
		Player_PGS ppgs = (Player_PGS) p1;
		ppgs.init2scripts();
		ppgs.setTimeLimit(tl);
		
		GUCTCD guctcdB = new GUCTCD(new UctConfig(1), 
				new ClusteringConfig(1, 6, new DynamicKMeans(30.0)));
		
		Player p2= new UctLogic(bwapi, guctcdB,tl);
	...
	public void pgs_vs_suct_tl(int tl){
	...
	public void poe2_vs_guct_tl(int tl){
		//timelimit is in ms
		Player p1= new Player_POE_final_no_cross(0);
		Player_POE_final_no_cross pe1 = (Player_POE_final_no_cross) p1;
		pe1.init2scripts();
	...
	public void poe6_vs_suct_tl(int tl){
		//timelimit is in ms
		Player p1= new Player_POE_final_no_cross(0);
		Player_POE_final_no_cross pe1 = (Player_POE_final_no_cross) p1;
		pe1.init6scripts();
	...
	public void poe2_vs_pgs_tl(int tl){
		//timelimit is in ms
		Player p1= new Player_POE_final_no_cross(0);
		Player_POE_final_no_cross pe1 = (Player_POE_final_no_cross) p1;
		pe1.init2scripts();
	...
	public void poe6_vs_suct_test_only(){
		Player p1= new Player_POE_final_no_cross(0);
		Player_POE_final_no_cross pe1 = (Player_POE_final_no_cross) p1;
		pe1.init6scripts();
	...
	public enum TestSetting{
		//for now only use all half dragoon + half zealot scenario
		D,DZ,Test; 
	}
	...
	private void ensureUnitNumSetting(Player p,int i){
		//there should be a better way of setting this up...
